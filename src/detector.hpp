...
class Detector {
 public:
  Detector(std::string model_file,std::string json_file, int epoch, int width, int height,
           float mean_r, float mean_g, float mean_b,
           int device_type=1, int device_id=0);

  std::vector<float> detect(zz::Image image);
  std::vector<float> detect(cv::Mat mat);
