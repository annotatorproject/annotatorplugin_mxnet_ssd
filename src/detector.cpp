...
Detector::Detector(std::string model_file,std::string json_file, int epoch, int width,
...
std::vector<float> Detector::detect(Image image)
{
    auto logger = log::get_logger("default");
    if (image.channels() != 3) {
...
std::vector<float> Detector::detect(cv::Mat mat)
{
    cimg_library::CImg<unsigned char> cimg(mat);
    return detect(det::cimg_to_zimage(cimg));
}
